...
export default function FetchUser() {
  ...
  return (
    <div style={{ maxWidth: 360 }}>
      ...
      {loading && (
        <p style={{ color: '#6b7280', fontSize: 14 }}>Cargando datos del lector...</p>
      )}
      {error && (
      ...
      {user && !loading && (
        <div style={{ padding: 14, border: '1px solid #e5e7eb', borderRadius: 8 }}>
          <p style={{ margin: '0 0 4px', fontWeight: 600 }}>Lector: {user.name}</p>
          ...
          <p style={{ margin: 0, fontSize: 13, color: '#6b7280' }}>
            Institución: {user.company.name} — {user.company.catchPhrase}
          </p>
        </div>
      )}
    </div>
